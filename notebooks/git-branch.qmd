...
With branching, practice and repitition is key to understanding.
Try not to panic.
:::
...
It isn't the ideal process to tackle the problem^[The start of an alternative could be `git revert --no-commit <hash>..HEAD; git commit`.] but it was chosen to highlight a few points.
