...
SSH can be a little harder to set up and has some advantages over HTTPS, but it less commonly used and can be problematic when firewalls are involved so we will stick to HTTPS.

But first we need to set up a Personal access token.
...
While SSH uses keys (along with a key agent which will normally already be on your machine) HTTPS relies on you having a mechanism to retain your credentials.
Neither having to type in your personal access token each time you want to push a change to a remote repository or retaining your credentials within the git config are viable nor recommended approaches.
...
Once it's installed and configured, the Git Credential Manager is called by git and you shouldn't need to do anything special.
